...
export function parseChecklistSpec(spec) {
  ...
  if (!nums.length) throw new Error(`Spec checklist inválido: "${spec}". Use all o 1,2,3`);
  return nums;
}
...
export async function updateIssueChecklist(issue, spec) {
  const parsed = parseChecklistSpec(typeof spec === "string" ? spec : spec);
