...
/**
 * Servicio para el ODS 13 - Acción por el Clima
...
export const objetivo13Service = {
  ...
  getIndicators: async (proyectoId) => {
    try {
      ...
      const response = await api.get(`/ods/13/indicadores`, { params: { proyectoId } });
      ...
      console.error('Error fetching ODS 13 indicators:', error);
      return {};
    }
  },

  // Estadísticas del ODS 13
  getStatistics: async () => {
    try {
      const response = await api.get(`/ods/13/estadisticas`);
      return response.data || {};
    } catch (error) {
      console.error('Error fetching ODS 13 statistics:', error);
      return {};
    }
  },

  // Dashboard específico del ODS 13 (V3)
  getDashboard: async () => {
    try {
      const response = await api.get(`/ods/13/dashboard`);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error fetching ODS 13 dashboard:', error);
  ...
  getIndicadorGeneral: async (code, proyectoId = 1) => {
    try {
      const response = await api.get(`/ods/13/indicadores/${code}`, { params: { proyectoId } });
  ...
  saveIndicator: async (indicatorData) => {
    try {
      ...
      console.error('Error saving ODS 13 indicator:', error);
  ...
  saveParameter: async (parameterData) => {
    try {
      ...
      console.error('Error saving ODS 13 parameter:', error);
...
,
  ...
  createMedicionAuditada: async ({ proyectoIndicadorId, fechaMedicion, responsable, metodoMedicion, observaciones, valoresParametros }) => {
    try {
      const res = await api.post('/ods/13/mediciones/auditada', {
        proyectoIndicadorId,
        fechaMedicion: fechaMedicion || new Date().toISOString().split('T')[0],
        responsable: responsable || 'Sistema',
  ...
  getMedicionAuditoria: async (medicionId) => {
    try {
      const res = await api.get(`/ods/13/mediciones/${medicionId}/auditoria`);
      return { success: true, data: res.data };
    } catch (e) {
