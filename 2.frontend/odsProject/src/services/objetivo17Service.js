...
export const objetivo17Service = {
  /**
   * Obtener todos los indicadores enriquecidos para un proyecto en el ODS 17
  ...
  getIndicators: async (proyectoId) => {
    try {
      ...
      const response = await api.get(`/ods/17/base-indicadores`, { params: { proyectoId } });
  ...
  /**
   * Obtiene estadísticas generales del ODS 17
   */
  getStatistics: async () => {
    try {
      const response = await api.get(`/ods/17/base-estadisticas`);
