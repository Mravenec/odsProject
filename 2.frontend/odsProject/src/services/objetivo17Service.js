...
/**
 * Servicio para el ODS 17 - Alianzas para Lograr los Objetivos
...
export const objetivo17Service = {
  ...
  getIndicators: async (proyectoId) => {
    try {
      ...
      const response = await api.get(`/ods/17/indicadores`, { params: { proyectoId } });
      ...
      console.error('Error fetching ODS 17 indicators:', error);
      return {};
    }
  },

  // Estadísticas del ODS 17
  getStatistics: async () => {
    try {
      const response = await api.get(`/ods/17/estadisticas`);
      return response.data || {};
    } catch (error) {
      console.error('Error fetching ODS 17 statistics:', error);
      return {};
    }
  },
  ...
  getIndicadorGeneral: async (code, proyectoId = 1) => {
    try {
      const response = await api.get(`/ods/17/indicadores/${code}`, { params: { proyectoId } });
  ...
  saveIndicator: async (indicatorData) => {
    try {
      ...
      const response = await api.post(`/ods/17/indicadores`, backendData);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error saving ODS 17 indicator:', error);
  ...
  saveParameter: async (parameterData) => {
    try {
      ...
      const response = await api.post(`/ods/17/metas`, backendData);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error saving ODS 17 parameter:', error);
...
,
  getMetasProyecto: async (proyectoId) => {
    try {
      const res = await api.get('/ods/17/metas', { params: { proyectoId } });
  ...
  getMediciones: async (proyectoIndicadorId) => {
    try {
      const res = await api.get('/ods/17/mediciones', { params: { indicadorId: proyectoIndicadorId } });
      return { success: true, data: res.data || [] };
  ...
  createMedicion: async ({ proyectoIndicadorId, valorCalculado, fechaMedicion, responsable }) => {
    try {
      const res = await api.post('/ods/17/mediciones', {
        proyectoIndicadorId,
        valorCalculado,
  ...
  createMedicionAuditada: async ({ proyectoIndicadorId, fechaMedicion, responsable, metodoMedicion, observaciones, valoresParametros }) => {
    try {
      const res = await api.post('/ods/17/mediciones/auditada', {
        proyectoIndicadorId,
        fechaMedicion: fechaMedicion || new Date().toISOString().split('T')[0],
        responsable: responsable || 'Sistema',
  ...
  getMedicionAuditoria: async (medicionId) => {
    try {
      const res = await api.get(`/ods/17/mediciones/${medicionId}/auditoria`);
      return { success: true, data: res.data };
    } catch (e) {
