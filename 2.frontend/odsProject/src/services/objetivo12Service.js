...
export const objetivo12Service = {
  /**
   * Obtener todos los indicadores enriquecidos para un proyecto en el ODS 12
  ...
  getIndicators: async (proyectoId) => {
    try {
      ...
      const response = await api.get(`/ods/12/base-indicadores`, { params: { proyectoId } });
  ...
  /**
   * Obtiene estadísticas generales del ODS 12
   */
  getStatistics: async () => {
    try {
      const response = await api.get(`/ods/12/base-estadisticas`);
