...
export const objetivo04Service = {
  /**
   * Obtener todos los indicadores enriquecidos para un proyecto en el ODS 04
  ...
  getIndicators: async (proyectoId) => {
    try {
      ...
      const response = await api.get(`/ods/04/base-indicadores`, { params: { proyectoId } });
  ...
  /**
   * Obtiene estadísticas generales del ODS 04
   */
  getStatistics: async () => {
    try {
      const response = await api.get(`/ods/04/base-estadisticas`);
