...
/**
 * Servicio para el ODS 04 - Educación de Calidad
...
export const objetivo04Service = {
  ...
  getIndicators: async (proyectoId) => {
    try {
      ...
      const response = await api.get(`/ods/04/indicadores`, { params: { proyectoId } });
      ...
      console.error('Error fetching ODS 04 indicators:', error);
      return {};
    }
  },

  // Estadísticas del ODS 04
  getStatistics: async () => {
    try {
      const response = await api.get(`/ods/04/estadisticas`);
      return response.data || {};
    } catch (error) {
      console.error('Error fetching ODS 04 statistics:', error);
      return {};
    }
  },

  // Dashboard específico del ODS 04 (V3)
  getDashboard: async () => {
    try {
      const response = await api.get(`/ods/04/dashboard`);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error fetching ODS 04 dashboard:', error);
  ...
  getIndicadorGeneral: async (code, proyectoId = 1) => {
    try {
      const response = await api.get(`/ods/04/indicadores/${code}`, { params: { proyectoId } });
  ...
  saveIndicator: async (indicatorData) => {
    try {
      ...
      const response = await api.post(`/ods/04/indicadores`, backendData);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error saving ODS 04 indicator:', error);
  ...
  saveParameter: async (parameterData) => {
    try {
      ...
      const response = await api.post(`/ods/04/metas`, backendData);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error saving ODS 04 parameter:', error);
...
,
  getMetasProyecto: async (proyectoId) => {
    try {
      const res = await api.get('/ods/04/metas', { params: { proyectoId } });
  ...
  getMediciones: async (proyectoIndicadorId) => {
    try {
      const res = await api.get('/ods/04/mediciones', { params: { indicadorId: proyectoIndicadorId } });
      return { success: true, data: res.data || [] };
  ...
  createMedicion: async ({ proyectoIndicadorId, valorCalculado, fechaMedicion, responsable }) => {
    try {
      const res = await api.post('/ods/04/mediciones', {
        proyectoIndicadorId,
        valorCalculado,
  ...
  createMedicionAuditada: async ({ proyectoIndicadorId, fechaMedicion, responsable, metodoMedicion, observaciones, valoresParametros }) => {
    try {
      const res = await api.post('/ods/04/mediciones/auditada', {
        proyectoIndicadorId,
        fechaMedicion: fechaMedicion || new Date().toISOString().split('T')[0],
        responsable: responsable || 'Sistema',
  ...
  getMedicionAuditoria: async (medicionId) => {
    try {
      const res = await api.get(`/ods/04/mediciones/${medicionId}/auditoria`);
      return { success: true, data: res.data };
    } catch (e) {
