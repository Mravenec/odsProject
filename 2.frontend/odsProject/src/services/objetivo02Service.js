...
export const objetivo02Service = {
  ...
  // Dashboard específico del ODS 02 (V3)
  getDashboard: async () => {
    try {
      const response = await api.get(`/ods/02/dashboard`);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error fetching ODS 02 dashboard:', error);
  ...
  saveIndicator: async (indicatorData) => {
    try {
      ...
      const response = await api.post(`/ods/02/indicadores`, backendData);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error saving ODS 02 indicator:', error);
  ...
  saveParameter: async (parameterData) => {
    try {
      ...
      const response = await api.post(`/ods/02/metas`, backendData);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error saving ODS 02 parameter:', error);
...
,
  getMetasProyecto: async (proyectoId) => {
    try {
      const res = await api.get('/ods/02/metas', { params: { proyectoId } });
  ...
  getMediciones: async (proyectoIndicadorId) => {
    try {
      const res = await api.get('/ods/02/mediciones', { params: { indicadorId: proyectoIndicadorId } });
      return { success: true, data: res.data || [] };
  ...
  createMedicion: async ({ proyectoIndicadorId, valorCalculado, fechaMedicion, responsable }) => {
    try {
      const res = await api.post('/ods/02/mediciones', {
        proyectoIndicadorId,
        valorCalculado,
  ...
  createMedicionAuditada: async ({ proyectoIndicadorId, fechaMedicion, responsable, metodoMedicion, observaciones, valoresParametros }) => {
    try {
      const res = await api.post('/ods/02/mediciones/auditada', {
        proyectoIndicadorId,
        fechaMedicion: fechaMedicion || new Date().toISOString().split('T')[0],
        responsable: responsable || 'Sistema',
  ...
  getMedicionAuditoria: async (medicionId) => {
    try {
      const res = await api.get(`/ods/02/mediciones/${medicionId}/auditoria`);
      return { success: true, data: res.data };
    } catch (e) {
