...
export const objetivo14Service = {
  ...
  getIndicators: async (proyectoId) => {
    try {
      ...
      const response = await api.get(`/ods/14/indicadores`, { params: { proyectoId } });
  ...
  // Estadísticas del ODS 14
  getStatistics: async () => {
    try {
      const response = await api.get(`/ods/14/estadisticas`);
  ...
  // Dashboard específico del ODS 14 (V3)
  getDashboard: async () => {
    try {
      const response = await api.get(`/ods/14/dashboard`);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error fetching ODS 14 dashboard:', error);
  ...
  saveIndicator: async (indicatorData) => {
    try {
      ...
      const response = await api.post(`/ods/14/indicadores`, backendData);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error saving ODS 14 indicator:', error);
  ...
  saveParameter: async (parameterData) => {
    try {
      ...
      const response = await api.post(`/ods/14/metas`, backendData);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error saving ODS 14 parameter:', error);
...
,
  getMetasProyecto: async (proyectoId) => {
    try {
      const res = await api.get('/ods/14/metas', { params: { proyectoId } });
  ...
  getMediciones: async (proyectoIndicadorId) => {
    try {
      const res = await api.get('/ods/14/mediciones', { params: { indicadorId: proyectoIndicadorId } });
      return { success: true, data: res.data || [] };
  ...
  createMedicion: async ({ proyectoIndicadorId, valorCalculado, fechaMedicion, responsable }) => {
    try {
      const res = await api.post('/ods/14/mediciones', {
        proyectoIndicadorId,
        valorCalculado,
  ...
  createMedicionAuditada: async ({ proyectoIndicadorId, fechaMedicion, responsable, metodoMedicion, observaciones, valoresParametros }) => {
    try {
      const res = await api.post('/ods/14/mediciones/auditada', {
        proyectoIndicadorId,
        fechaMedicion: fechaMedicion || new Date().toISOString().split('T')[0],
        responsable: responsable || 'Sistema',
  ...
  getMedicionAuditoria: async (medicionId) => {
    try {
      const res = await api.get(`/ods/14/mediciones/${medicionId}/auditoria`);
      return { success: true, data: res.data };
    } catch (e) {
