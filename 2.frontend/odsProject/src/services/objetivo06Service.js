...
export const objetivo06Service = {
  /**
   * Obtener todos los indicadores enriquecidos para un proyecto en el ODS 06
  ...
  getIndicators: async (proyectoId) => {
    try {
      ...
      const response = await api.get(`/ods/06/base-indicadores`, { params: { proyectoId } });
  ...
  /**
   * Obtiene estadísticas generales del ODS 06
   */
  getStatistics: async () => {
    try {
      const response = await api.get(`/ods/06/base-estadisticas`);
