...
export const objetivo11Service = {
  ...
  // Dashboard específico del ODS 11 (V3)
  getDashboard: async () => {
    try {
      const response = await api.get(`/ods/11/dashboard`);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error fetching ODS 11 dashboard:', error);
  ...
  saveIndicator: async (indicatorData) => {
    try {
      ...
      const response = await api.post(`/ods/11/indicadores`, backendData);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error saving ODS 11 indicator:', error);
  ...
  saveParameter: async (parameterData) => {
    try {
      ...
      const response = await api.post(`/ods/11/metas`, backendData);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error saving ODS 11 parameter:', error);
...
,
  getMetasProyecto: async (proyectoId) => {
    try {
      const res = await api.get('/ods/11/metas', { params: { proyectoId } });
  ...
  getMediciones: async (proyectoIndicadorId) => {
    try {
      const res = await api.get('/ods/11/mediciones', { params: { indicadorId: proyectoIndicadorId } });
      return { success: true, data: res.data || [] };
  ...
  createMedicion: async ({ proyectoIndicadorId, valorCalculado, fechaMedicion, responsable }) => {
    try {
      const res = await api.post('/ods/11/mediciones', {
        proyectoIndicadorId,
        valorCalculado,
