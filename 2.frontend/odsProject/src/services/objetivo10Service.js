...
export const objetivo10Service = {
  /**
   * Obtener todos los indicadores enriquecidos para un proyecto en el ODS 10
  ...
  getIndicators: async (proyectoId) => {
    try {
      ...
      const response = await api.get(`/ods/10/base-indicadores`, { params: { proyectoId } });
  ...
  /**
   * Obtiene estadísticas generales del ODS 10
   */
  getStatistics: async () => {
    try {
      const response = await api.get(`/ods/10/base-estadisticas`);
