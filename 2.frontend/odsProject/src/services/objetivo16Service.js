...
export const objetivo16Service = {
  /**
   * Obtener todos los indicadores enriquecidos para un proyecto en el ODS 16
  ...
  getIndicators: async (proyectoId) => {
    try {
      ...
      const response = await api.get(`/ods/16/base-indicadores`, { params: { proyectoId } });
  ...
  /**
   * Obtiene estadísticas generales del ODS 16
   */
  getStatistics: async () => {
    try {
      const response = await api.get(`/ods/16/base-estadisticas`);
