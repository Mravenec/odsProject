...
/**
 * Servicio para el ODS 16 - Paz, Justicia e Instituciones Sólidas
...
export const objetivo16Service = {
  ...
  getIndicators: async (proyectoId) => {
    try {
      ...
      const response = await api.get(`/ods/16/indicadores`, { params: { proyectoId } });
      ...
      console.error('Error fetching ODS 16 indicators:', error);
      return {};
    }
  },

  // Estadísticas del ODS 16
  getStatistics: async () => {
    try {
      const response = await api.get(`/ods/16/estadisticas`);
      return response.data || {};
    } catch (error) {
      console.error('Error fetching ODS 16 statistics:', error);
      return {};
    }
  },

  // Dashboard específico del ODS 16 (V3)
  getDashboard: async () => {
    try {
      const response = await api.get(`/ods/16/dashboard`);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error fetching ODS 16 dashboard:', error);
  ...
  getIndicadorGeneral: async (code, proyectoId = 1) => {
    try {
      const response = await api.get(`/ods/16/indicadores/${code}`, { params: { proyectoId } });
  ...
  saveIndicator: async (indicatorData) => {
    try {
      ...
      const response = await api.post(`/ods/16/indicadores`, backendData);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error saving ODS 16 indicator:', error);
  ...
  saveParameter: async (parameterData) => {
    try {
      ...
      const response = await api.post(`/ods/16/metas`, backendData);
      return { success: true, data: response.data };
    } catch (error) {
      console.error('Error saving ODS 16 parameter:', error);
...
,
  getMetasProyecto: async (proyectoId) => {
    try {
      const res = await api.get('/ods/16/metas', { params: { proyectoId } });
  ...
  getMediciones: async (proyectoIndicadorId) => {
    try {
      const res = await api.get('/ods/16/mediciones', { params: { indicadorId: proyectoIndicadorId } });
      return { success: true, data: res.data || [] };
  ...
  createMedicion: async ({ proyectoIndicadorId, valorCalculado, fechaMedicion, responsable }) => {
    try {
      const res = await api.post('/ods/16/mediciones', {
        proyectoIndicadorId,
        valorCalculado,
  ...
  createMedicionAuditada: async ({ proyectoIndicadorId, fechaMedicion, responsable, metodoMedicion, observaciones, valoresParametros }) => {
    try {
      const res = await api.post('/ods/16/mediciones/auditada', {
        proyectoIndicadorId,
        fechaMedicion: fechaMedicion || new Date().toISOString().split('T')[0],
        responsable: responsable || 'Sistema',
  ...
  getMedicionAuditoria: async (medicionId) => {
    try {
      const res = await api.get(`/ods/16/mediciones/${medicionId}/auditoria`);
      return { success: true, data: res.data };
    } catch (e) {
