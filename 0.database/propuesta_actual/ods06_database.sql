...
-- Esta base de datos se enfoca exclusivamente en la gestión de proyectos ODS06.
...
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_ods06 (tabla_afectada, registro_id, accion, usuario_id, valores_anteriores, valores_nuevos)
...
-- ────────────────────────────────────────────────────────────
-- VISTA: Estadísticas Generales ODS06 (metas específicas)
-- ────────────────────────────────────────────────────────────

CREATE VIEW vista_admin_estadisticas_generales AS
SELECT
    '6.1' AS meta_codigo,
    'Lograr acceso universal equitativo agua potable asequible' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '6.1.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '6.1.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_6_1,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '6.1.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_6_1_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 6

UNION ALL
SELECT
    '6.2' AS meta_codigo,
    'Lograr acceso saneamiento higiene adecuados equitativos' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '6.2.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '6.2.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_6_2,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '6.2.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_6_2_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 6
...
SELECT
    '6.3' AS meta_codigo,
    'Mejorar calidad agua reducir contaminación vertimiento' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '6.3.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '6.3.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_6_3,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '6.3.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_6_3_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 6
...
SELECT
    '6.4' AS meta_codigo,
    'Aumentar uso eficiente recursos hídricos todos sectores' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '6.4.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '6.4.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_6_4,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '6.4.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_6_4_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 6
...
SELECT
    '6.5' AS meta_codigo,
    'Implementar gestión integrada recursos hídricos todos niveles' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '6.5.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '6.5.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_6_5,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '6.5.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_6_5_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 6
...
SELECT
    '6.6' AS meta_codigo,
    'Proteger restablecer ecosistemas relacionados agua' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '6.6.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '6.6.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_6_6,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '6.6.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_6_6_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 6
...
SELECT
    '6.a' AS meta_codigo,
    'Ampliar cooperación internacional apoyo países en desarrollo agua saneamiento' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '6.a.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '6.a.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_6_a,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '6.a.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_6_a_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 6
...
SELECT
    '6.b' AS meta_codigo,
    'Apoyar fortalecer participación comunidades locales gestión agua saneamiento' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '6.b.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '6.b.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_6_b,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '6.b.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_6_b_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 6;
...
BEGIN
    SELECT 'TOTAL_PROYECTOS'     AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 6
    UNION ALL
    SELECT 'PROYECTOS_ACTIVOS'   AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 6 AND estado = 'activo'
    UNION ALL
    SELECT 'PROYECTOS_COMPLETADOS' AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 6 AND estado = 'completado'
    UNION ALL
    ...
    FROM indicadores i
    JOIN proyectos p ON i.proyecto_id = p.id
    WHERE p.objetivo_id = 6 AND (i.valor_actual / i.valor_meta) < 0.5
...
BEGIN
    ...
    FROM proyectos p
    JOIN ods_login.usuarios u ON p.usuario_id = u.id
    WHERE p.id = proyecto_id_param AND p.objetivo_id = 6;
...
ALTER TABLE proyectos             COMMENT 'Proyectos ODS06 creados por usuarios';
ALTER TABLE metas_proyecto        COMMENT 'Metas específicas establecidas por cada proyecto ODS06';
ALTER TABLE indicadores           COMMENT 'Indicadores medidos por cada proyecto ODS06';
ALTER TABLE mediciones_historicas COMMENT 'Historial de mediciones de indicadores ODS06';
ALTER TABLE auditoria_ods06         COMMENT 'Auditoría de cambios en el sistema ODS06';

SELECT 'Base de datos ODS06 creada exitosamente' AS mensaje, NOW() AS fecha_creacion;
