...
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_ods01 (tabla_afectada, registro_id, accion, usuario_id, valores_anteriores, valores_nuevos)
...
-- ────────────────────────────────────────────────────────────
-- VISTA: Estadísticas Generales ODS01 (metas específicas)
-- ────────────────────────────────────────────────────────────

CREATE VIEW vista_admin_estadisticas_generales AS
SELECT
    '1.1' AS meta_codigo,
    'Erradicar pobreza externa mundo' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '1.1.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '1.1.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_1_1,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '1.1.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_1_1_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 1

UNION ALL
SELECT
    '1.2' AS meta_codigo,
    'Reducir proporción personas viven pobreza todas dimensiones' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '1.2.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '1.2.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_1_2,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '1.2.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_1_2_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 1
...
SELECT
    '1.3' AS meta_codigo,
    'Implementar sistemas protección social todos' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '1.3.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '1.3.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_1_3,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '1.3.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_1_3_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 1
...
SELECT
    '1.4' AS meta_codigo,
    'Garantizar mismos derechos recursos económicos acceso servicios básicos' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '1.4.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '1.4.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_1_4,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '1.4.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_1_4_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 1
...
SELECT
    '1.5' AS meta_codigo,
    'Fomentar resiliencia pobres vulnerables desastres económicos ambientales' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '1.5.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '1.5.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_1_5,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '1.5.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_1_5_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 1
...
SELECT
    '1.a' AS meta_codigo,
    'Garantizar movilización recursos desarrollo países en desarrollo' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '1.a.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '1.a.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_1_a,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '1.a.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_1_a_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 1
...
SELECT
    '1.b' AS meta_codigo,
    'Crear marcos normativos sólidos desarrollo favorables pobres' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '1.b.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '1.b.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_1_b,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '1.b.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_1_b_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 1;
...
BEGIN
    SELECT 'TOTAL_PROYECTOS'     AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 1
    UNION ALL
    SELECT 'PROYECTOS_ACTIVOS'   AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 1 AND estado = 'activo'
    UNION ALL
    SELECT 'PROYECTOS_COMPLETADOS' AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 1 AND estado = 'completado'
    UNION ALL
    ...
    FROM indicadores i
    JOIN proyectos p ON i.proyecto_id = p.id
    WHERE p.objetivo_id = 1 AND (i.valor_actual / i.valor_meta) < 0.5
...
BEGIN
    ...
    FROM proyectos p
    JOIN ods_login.usuarios u ON p.usuario_id = u.id
    WHERE p.id = proyecto_id_param AND p.objetivo_id = 1;
...
ALTER TABLE proyectos             COMMENT 'Proyectos ODS01 creados por usuarios';
ALTER TABLE metas_proyecto        COMMENT 'Metas específicas establecidas por cada proyecto ODS01';
ALTER TABLE indicadores           COMMENT 'Indicadores medidos por cada proyecto ODS01';
ALTER TABLE mediciones_historicas COMMENT 'Historial de mediciones de indicadores ODS01';
ALTER TABLE auditoria_ods01         COMMENT 'Auditoría de cambios en el sistema ODS01';

SELECT 'Base de datos ODS01 creada exitosamente' AS mensaje, NOW() AS fecha_creacion;
