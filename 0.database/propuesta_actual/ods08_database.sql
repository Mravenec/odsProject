...
-- Esta base de datos se enfoca exclusivamente en la gestión de proyectos ODS08.
...
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_ods08 (tabla_afectada, registro_id, accion, usuario_id, valores_anteriores, valores_nuevos)
...
-- ────────────────────────────────────────────────────────────
-- VISTA: Estadísticas Generales ODS08 (metas específicas)
-- ────────────────────────────────────────────────────────────

CREATE VIEW vista_admin_estadisticas_generales AS
SELECT
    '8.1' AS meta_codigo,
    'Mantener crecimiento económico per cápita' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.1.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '8.1.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_8_1,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.1.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_8_1_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 8

UNION ALL
SELECT
    '8.2' AS meta_codigo,
    'Lograr niveles productividad económica diversificación' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.2.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '8.2.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_8_2,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.2.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_8_2_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 8
...
SELECT
    '8.3' AS meta_codigo,
    'Promover políticas desarrollo actividades productivas' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.3.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '8.3.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_8_3,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.3.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_8_3_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 8
...
SELECT
    '8.4' AS meta_codigo,
    'Mejorar producción consumo eficientes recursos' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.4.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '8.4.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_8_4,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.4.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_8_4_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 8
...
SELECT
    '8.5' AS meta_codigo,
    'Lograr empleo pleno productivo trabajo decente' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.5.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '8.5.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_8_5,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.5.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_8_5_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 8
...
SELECT
    '8.6' AS meta_codigo,
    'Reducir proporción jóvenes no estudian no trabajan' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.6.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '8.6.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_8_6,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.6.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_8_6_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 8
...
SELECT
    '8.7' AS meta_codigo,
    'Erradicar trabajo forzoso esclavitud trata personas' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.7.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '8.7.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_8_7,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.7.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_8_7_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 8
...
SELECT
    '8.8' AS meta_codigo,
    'Proteger derechos laborales entorno trabajo seguro' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.8.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '8.8.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_8_8,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.8.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_8_8_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 8
...
SELECT
    '8.9' AS meta_codigo,
    'Promover turismo sostenible crea puestos trabajo' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.9.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '8.9.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_8_9,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.9.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_8_9_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 8
...
SELECT
    '8.10' AS meta_codigo,
    'Fortalecer capacidad instituciones financieras' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.10.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '8.10.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_8_10,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.10.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_8_10_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 8
...
SELECT
    '8.a' AS meta_codigo,
    'Aumentar apoyo iniciativa ayuda comercio países en desarrollo' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.a.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '8.a.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_8_a,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.a.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_8_a_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 8
...
SELECT
    '8.b' AS meta_codigo,
    'Desarrollar estrategia mundial empleo jóvenes' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.b.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '8.b.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_8_b,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '8.b.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_8_b_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 8;
...
BEGIN
    SELECT 'TOTAL_PROYECTOS'     AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 8
    UNION ALL
    SELECT 'PROYECTOS_ACTIVOS'   AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 8 AND estado = 'activo'
    UNION ALL
    SELECT 'PROYECTOS_COMPLETADOS' AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 8 AND estado = 'completado'
    UNION ALL
    ...
    FROM indicadores i
    JOIN proyectos p ON i.proyecto_id = p.id
    WHERE p.objetivo_id = 8 AND (i.valor_actual / i.valor_meta) < 0.5
...
BEGIN
    ...
    FROM proyectos p
    JOIN ods_login.usuarios u ON p.usuario_id = u.id
    WHERE p.id = proyecto_id_param AND p.objetivo_id = 8;
...
CREATE INDEX idx_auditoria_fecha_tabla ON auditoria_ods08(fecha_cambio, tabla_afectada);

ALTER TABLE proyectos             COMMENT 'Proyectos ODS08 creados por usuarios';
ALTER TABLE metas_proyecto        COMMENT 'Metas específicas establecidas por cada proyecto ODS08';
ALTER TABLE indicadores           COMMENT 'Indicadores medidos por cada proyecto ODS08';
ALTER TABLE mediciones_historicas COMMENT 'Historial de mediciones de indicadores ODS08';
ALTER TABLE auditoria_ods08         COMMENT 'Auditoría de cambios en el sistema ODS08';

SELECT 'Base de datos ODS08 creada exitosamente' AS mensaje, NOW() AS fecha_creacion;
