...
-- Esta base de datos se enfoca exclusivamente en la gestión de proyectos ODS09.
...
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_ods09 (tabla_afectada, registro_id, accion, usuario_id, valores_anteriores, valores_nuevos)
...
-- ────────────────────────────────────────────────────────────
-- VISTA: Estadísticas Generales ODS09 (metas específicas)
-- ────────────────────────────────────────────────────────────

CREATE VIEW vista_admin_estadisticas_generales AS
SELECT
    '9.1' AS meta_codigo,
    'Construir infraestructura resiliente promover industrialización inclusiva' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '9.1.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '9.1.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_9_1,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '9.1.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_9_1_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 9

UNION ALL
SELECT
    '9.2' AS meta_codigo,
    'Promover industrialización inclusiva sostenible' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '9.2.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '9.2.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_9_2,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '9.2.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_9_2_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 9
...
SELECT
    '9.3' AS meta_codigo,
    'Aumentar acceso servicios financieros pequeñas industrias' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '9.3.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '9.3.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_9_3,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '9.3.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_9_3_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 9
...
SELECT
    '9.4' AS meta_codigo,
    'Modernizar infraestructura reindustrializar tecnologías limpias' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '9.4.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '9.4.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_9_4,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '9.4.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_9_4_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 9
...
SELECT
    '9.5' AS meta_codigo,
    'Aumentar investigación científica mejorar capacidad tecnológica' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '9.5.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '9.5.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_9_5,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '9.5.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_9_5_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 9
...
SELECT
    '9.a' AS meta_codigo,
    'Facilitar desarrollo infraestructura sostenible resiliente países en desarrollo' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '9.a.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '9.a.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_9_a,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '9.a.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_9_a_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 9
...
SELECT
    '9.b' AS meta_codigo,
    'Apoyar desarrollo tecnología investigación innovación países en desarrollo' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '9.b.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '9.b.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_9_b,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '9.b.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_9_b_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 9
...
SELECT
    '9.c' AS meta_codigo,
    'Aumentar acceso tecnología comunicaciones Internet' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '9.c.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '9.c.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_9_c,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '9.c.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_9_c_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 9;
...
BEGIN
    SELECT 'TOTAL_PROYECTOS'     AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 9
    UNION ALL
    SELECT 'PROYECTOS_ACTIVOS'   AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 9 AND estado = 'activo'
    UNION ALL
    SELECT 'PROYECTOS_COMPLETADOS' AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 9 AND estado = 'completado'
    UNION ALL
    ...
    FROM indicadores i
    JOIN proyectos p ON i.proyecto_id = p.id
    WHERE p.objetivo_id = 9 AND (i.valor_actual / i.valor_meta) < 0.5
...
BEGIN
    ...
    FROM proyectos p
    JOIN ods_login.usuarios u ON p.usuario_id = u.id
    WHERE p.id = proyecto_id_param AND p.objetivo_id = 9;
...
CREATE INDEX idx_auditoria_fecha_tabla ON auditoria_ods09(fecha_cambio, tabla_afectada);

ALTER TABLE proyectos             COMMENT 'Proyectos ODS09 creados por usuarios';
ALTER TABLE metas_proyecto        COMMENT 'Metas específicas establecidas por cada proyecto ODS09';
ALTER TABLE indicadores           COMMENT 'Indicadores medidos por cada proyecto ODS09';
ALTER TABLE mediciones_historicas COMMENT 'Historial de mediciones de indicadores ODS09';
ALTER TABLE auditoria_ods09         COMMENT 'Auditoría de cambios en el sistema ODS09';

SELECT 'Base de datos ODS09 creada exitosamente' AS mensaje, NOW() AS fecha_creacion;
