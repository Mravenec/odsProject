...
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_ods07 (tabla_afectada, registro_id, accion, usuario_id, valores_anteriores, valores_nuevos)
...
-- ────────────────────────────────────────────────────────────
-- VISTA: Estadísticas Generales ODS07 (metas específicas)
-- ────────────────────────────────────────────────────────────
...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '7.1.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_7_1_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '7.2.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_7_2_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 7
...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '7.3.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_7_3_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 7
...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '7.a.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_7_a_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 7
...
SELECT
    ...
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '7.b.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_7_b_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 7;
...
BEGIN
    SELECT 'TOTAL_PROYECTOS'     AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 7
    UNION ALL
    SELECT 'PROYECTOS_ACTIVOS'   AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 7 AND estado = 'activo'
    UNION ALL
    SELECT 'PROYECTOS_COMPLETADOS' AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 7 AND estado = 'completado'
    UNION ALL
    ...
    FROM indicadores i
    JOIN proyectos p ON i.proyecto_id = p.id
    WHERE p.objetivo_id = 7 AND (i.valor_actual / i.valor_meta) < 0.5
...
CREATE INDEX idx_auditoria_fecha_tabla ON auditoria_ods07(fecha_cambio, tabla_afectada);
...
ALTER TABLE auditoria_ods07         COMMENT 'Auditoría de cambios en el sistema ODS07';
