...
-- Esta base de datos se enfoca exclusivamente en la gestión de proyectos ODS13.
...
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_ods13 (tabla_afectada, registro_id, accion, usuario_id, valores_anteriores, valores_nuevos)
...
-- ────────────────────────────────────────────────────────────
-- VISTA: Estadísticas Generales ODS13 (metas específicas)
-- ────────────────────────────────────────────────────────────

CREATE VIEW vista_admin_estadisticas_generales AS
SELECT
    '13.1' AS meta_codigo,
    'Fortalecer resiliencia y adaptación al clima' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '13.1.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '13.1.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_13_1,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '13.1.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_13_1_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 13

UNION ALL
SELECT
    '13.2' AS meta_codigo,
    'Incorporar medidas cambio climático políticas nacionales' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '13.2.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '13.2.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_13_2,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '13.2.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_13_2_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 13
...
SELECT
    '13.3' AS meta_codigo,
    'Mejorar educación sensibilización cambio climático' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '13.3.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '13.3.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_13_3,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '13.3.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_13_3_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 13
...
SELECT
    '13.4' AS meta_codigo,
    'Promover prácticas adquisiciones públicas sostenibles' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '13.4.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '13.4.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_13_4,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '13.4.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_13_4_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 13
...
SELECT
    '13.a' AS meta_codigo,
    'Cumplir compromiso países desarrollados cambio climático' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '13.a.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '13.a.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_13_a,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '13.a.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_13_a_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 13
...
SELECT
    '13.b' AS meta_codigo,
    'Promover mecanismos capacidad planificación gestión climática' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '13.b.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '13.b.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_13_b,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '13.b.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_13_b_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 13;
...
BEGIN
    SELECT 'TOTAL_PROYECTOS'     AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 13
    UNION ALL
    SELECT 'PROYECTOS_ACTIVOS'   AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 13 AND estado = 'activo'
    UNION ALL
    SELECT 'PROYECTOS_COMPLETADOS' AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 13 AND estado = 'completado'
    UNION ALL
    ...
    FROM indicadores i
    JOIN proyectos p ON i.proyecto_id = p.id
    WHERE p.objetivo_id = 13 AND (i.valor_actual / i.valor_meta) < 0.5
...
BEGIN
    ...
    FROM proyectos p
    JOIN ods_login.usuarios u ON p.usuario_id = u.id
    WHERE p.id = proyecto_id_param AND p.objetivo_id = 13;
...
CREATE INDEX idx_auditoria_fecha_tabla ON auditoria_ods13(fecha_cambio, tabla_afectada);

ALTER TABLE proyectos             COMMENT 'Proyectos ODS13 creados por usuarios';
ALTER TABLE metas_proyecto        COMMENT 'Metas específicas establecidas por cada proyecto ODS13';
ALTER TABLE indicadores           COMMENT 'Indicadores medidos por cada proyecto ODS13';
ALTER TABLE mediciones_historicas COMMENT 'Historial de mediciones de indicadores ODS13';
ALTER TABLE auditoria_ods13         COMMENT 'Auditoría de cambios en el sistema ODS13';

SELECT 'Base de datos ODS13 creada exitosamente' AS mensaje, NOW() AS fecha_creacion;
