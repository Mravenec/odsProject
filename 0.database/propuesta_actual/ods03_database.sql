...
-- Esta base de datos se enfoca exclusivamente en la gestión de proyectos ODS03.
...
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_ods03 (tabla_afectada, registro_id, accion, usuario_id, valores_anteriores, valores_nuevos)
...
-- ────────────────────────────────────────────────────────────
-- VISTA: Estadísticas Generales ODS03 (metas específicas)
-- ────────────────────────────────────────────────────────────

CREATE VIEW vista_admin_estadisticas_generales AS
SELECT
    '3.1' AS meta_codigo,
    'Reducir mortalidad materna global' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.1.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '3.1.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_3_1,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.1.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_3_1_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 3

UNION ALL
SELECT
    '3.2' AS meta_codigo,
    'Poner fin muertes menores 5 años enfermedades prevenibles' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.2.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '3.2.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_3_2,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.2.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_3_2_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 3
...
SELECT
    '3.3' AS meta_codigo,
    'Combatir enfermedades transmisibles epidemias' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.3.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '3.3.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_3_3,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.3.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_3_3_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 3
...
SELECT
    '3.4' AS meta_codigo,
    'Reducir mortalidad enfermedades crónicas' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.4.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '3.4.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_3_4,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.4.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_3_4_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 3
...
SELECT
    '3.5' AS meta_codigo,
    'Prevenir tratar adicción sustancias trastornos abuso' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.5.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '3.5.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_3_5,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.5.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_3_5_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 3
...
SELECT
    '3.6' AS meta_codigo,
    'Reducir mortalidad accidentes tráfico global' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.6.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '3.6.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_3_6,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.6.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_3_6_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 3
...
SELECT
    '3.7' AS meta_codigo,
    'Acceso servicios salud sexual reproductiva' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.7.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '3.7.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_3_7,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.7.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_3_7_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 3
...
SELECT
    '3.8' AS meta_codigo,
    'Lograr acceso servicios salud esenciales' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.8.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '3.8.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_3_8,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.8.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_3_8_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 3
...
SELECT
    '3.9' AS meta_codigo,
    'Reducir muertes contaminación agua aire ambiente' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.9.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '3.9.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_3_9,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.9.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_3_9_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 3
...
SELECT
    '3.a' AS meta_codigo,
    'Fortalecer implementación Convenio Marco Control Tabaco' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.a.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '3.a.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_3_a,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.a.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_3_a_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 3
...
SELECT
    '3.b' AS meta_codigo,
    'Apoyar investigación desarrollo medicamentos vacunas' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.b.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '3.b.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_3_b,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.b.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_3_b_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 3
...
SELECT
    '3.c' AS meta_codigo,
    'Aumentar densidad distribución personal sanitario' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.c.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '3.c.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_3_c,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.c.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_3_c_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 3
...
SELECT
    '3.d' AS meta_codigo,
    'Fortalecer capacidad sistemas alerta emergencias sanitarias' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.d.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '3.d.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_3_d,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '3.d.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_3_d_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 3;
...
BEGIN
    SELECT 'TOTAL_PROYECTOS'     AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 3
    UNION ALL
    SELECT 'PROYECTOS_ACTIVOS'   AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 3 AND estado = 'activo'
    UNION ALL
    SELECT 'PROYECTOS_COMPLETADOS' AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 3 AND estado = 'completado'
    UNION ALL
    ...
    FROM indicadores i
    JOIN proyectos p ON i.proyecto_id = p.id
    WHERE p.objetivo_id = 3 AND (i.valor_actual / i.valor_meta) < 0.5
...
BEGIN
    ...
    FROM proyectos p
    JOIN ods_login.usuarios u ON p.usuario_id = u.id
    WHERE p.id = proyecto_id_param AND p.objetivo_id = 3;
...
ALTER TABLE proyectos             COMMENT 'Proyectos ODS03 creados por usuarios';
ALTER TABLE metas_proyecto        COMMENT 'Metas específicas establecidas por cada proyecto ODS03';
ALTER TABLE indicadores           COMMENT 'Indicadores medidos por cada proyecto ODS03';
ALTER TABLE mediciones_historicas COMMENT 'Historial de mediciones de indicadores ODS03';
ALTER TABLE auditoria_ods03         COMMENT 'Auditoría de cambios en el sistema ODS03';

SELECT 'Base de datos ODS03 creada exitosamente' AS mensaje, NOW() AS fecha_creacion;
