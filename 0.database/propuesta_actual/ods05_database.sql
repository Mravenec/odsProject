...
-- Esta base de datos se enfoca exclusivamente en la gestión de proyectos ODS05.
...
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_ods05 (tabla_afectada, registro_id, accion, usuario_id, valores_anteriores, valores_nuevos)
...
-- ────────────────────────────────────────────────────────────
-- VISTA: Estadísticas Generales ODS05 (metas específicas)
-- ────────────────────────────────────────────────────────────

CREATE VIEW vista_admin_estadisticas_generales AS
SELECT
    '5.1' AS meta_codigo,
    'Poner fin discriminación contra mujeres niñas mundo' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.1.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '5.1.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_5_1,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.1.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_5_1_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 5

UNION ALL
SELECT
    '5.2' AS meta_codigo,
    'Eliminar violencia contra mujeres niñas ámbitos público privado' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.2.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '5.2.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_5_2,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.2.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_5_2_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 5
...
SELECT
    '5.3' AS meta_codigo,
    'Eliminar prácticas nocivas matrimonio infantil mutilación genital' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.3.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '5.3.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_5_3,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.3.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_5_3_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 5
...
SELECT
    '5.4' AS meta_codigo,
    'Reconocer valorar cuidados trabajo doméstico no remunerado' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.4.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '5.4.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_5_4,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.4.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_5_4_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 5
...
SELECT
    '5.5' AS meta_codigo,
    'Asegurar participación plena efectiva mujeres liderazgo' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.5.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '5.5.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_5_5,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.5.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_5_5_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 5
...
SELECT
    '5.6' AS meta_codigo,
    'Asegurar acceso universal salud sexual reproductiva' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.6.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '5.6.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_5_6,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.6.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_5_6_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 5
...
SELECT
    '5.a' AS meta_codigo,
    'Emprender reformas otorgar mujeres igualdad derechos económicos' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.a.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '5.a.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_5_a,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.a.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_5_a_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 5
...
SELECT
    '5.b' AS meta_codigo,
    'Mejorar uso tecnología instrumental promover empoderamiento mujeres' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.b.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '5.b.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_5_b,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.b.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_5_b_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 5
...
SELECT
    '5.c' AS meta_codigo,
    'Aprobar fortalecer políticas leyes igualdad género empoderamiento' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.c.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '5.c.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_5_c,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '5.c.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_5_c_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 5;
...
BEGIN
    SELECT 'TOTAL_PROYECTOS'     AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 5
    UNION ALL
    SELECT 'PROYECTOS_ACTIVOS'   AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 5 AND estado = 'activo'
    UNION ALL
    SELECT 'PROYECTOS_COMPLETADOS' AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 5 AND estado = 'completado'
    UNION ALL
    ...
    FROM indicadores i
    JOIN proyectos p ON i.proyecto_id = p.id
    WHERE p.objetivo_id = 5 AND (i.valor_actual / i.valor_meta) < 0.5
...
BEGIN
    ...
    FROM proyectos p
    JOIN ods_login.usuarios u ON p.usuario_id = u.id
    WHERE p.id = proyecto_id_param AND p.objetivo_id = 5;
...
ALTER TABLE proyectos             COMMENT 'Proyectos ODS05 creados por usuarios';
ALTER TABLE metas_proyecto        COMMENT 'Metas específicas establecidas por cada proyecto ODS05';
ALTER TABLE indicadores           COMMENT 'Indicadores medidos por cada proyecto ODS05';
ALTER TABLE mediciones_historicas COMMENT 'Historial de mediciones de indicadores ODS05';
ALTER TABLE auditoria_ods05         COMMENT 'Auditoría de cambios en el sistema ODS05';

SELECT 'Base de datos ODS05 creada exitosamente' AS mensaje, NOW() AS fecha_creacion;
