...
-- Esta base de datos se enfoca exclusivamente en la gestión de proyectos ODS16.
...
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_ods16 (tabla_afectada, registro_id, accion, usuario_id, valores_anteriores, valores_nuevos)
...
-- ────────────────────────────────────────────────────────────
-- VISTA: Estadísticas Generales ODS16 (metas específicas)
-- ────────────────────────────────────────────────────────────

CREATE VIEW vista_admin_estadisticas_generales AS
SELECT
    '16.1' AS meta_codigo,
    'Reducir significativamente todas las formas de violencia' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.1.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '16.1.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_16_1,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.1.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_16_1_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
...
SELECT
    '16.2' AS meta_codigo,
    'Poner fin al maltrato, la explotación, la trata' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.2.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '16.2.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_16_2,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.2.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_16_2_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 16
...
SELECT
    '16.3' AS meta_codigo,
    'Promover el estado de derecho' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.3.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '16.3.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_16_3,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.3.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_16_3_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 16
...
SELECT
    '16.4' AS meta_codigo,
    'Reducir corrientes financieras ilícitas' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.4.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '16.4.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_16_4,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.4.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_16_4_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 16
...
SELECT
    '16.5' AS meta_codigo,
    'Reducir considerablemente la corrupción' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.5.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '16.5.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_16_5,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.5.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_16_5_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 16
...
SELECT
    '16.6' AS meta_codigo,
    'Crear instituciones eficaces y transparentes' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.6.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '16.6.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_16_6,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.6.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_16_6_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 16
...
SELECT
    '16.7' AS meta_codigo,
    'Garantizar decisiones inclusivas' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.7.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '16.7.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_16_7,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.7.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_16_7_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 16
...
SELECT
    '16.8' AS meta_codigo,
    'Ampliar participación países en desarrollo' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.8.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '16.8.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_16_8,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.8.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_16_8_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 16
...
SELECT
    '16.9' AS meta_codigo,
    'Proporcionar acceso a identidad jurídica' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.9.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '16.9.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_16_9,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.9.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_16_9_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 16
...
UNION ALL
SELECT
    '16.10' AS meta_codigo,
    'Garantizar acceso público a información' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.10.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '16.10.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_16_10,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '16.10.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_16_10_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 16;
...
BEGIN
    SELECT 'TOTAL_PROYECTOS'     AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 16
    UNION ALL
    SELECT 'PROYECTOS_ACTIVOS'   AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 16 AND estado = 'activo'
    UNION ALL
    SELECT 'PROYECTOS_COMPLETADOS' AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 16 AND estado = 'completado'
    UNION ALL
    ...
    FROM indicadores i
    JOIN proyectos p ON i.proyecto_id = p.id
    WHERE p.objetivo_id = 16 AND (i.valor_actual / i.valor_meta) < 0.5
...
CREATE INDEX idx_auditoria_fecha_tabla ON auditoria_ods16(fecha_cambio, tabla_afectada);

ALTER TABLE proyectos             COMMENT 'Proyectos ODS16 creados por usuarios';
ALTER TABLE metas_proyecto        COMMENT 'Metas específicas establecidas por cada proyecto ODS16';
ALTER TABLE indicadores           COMMENT 'Indicadores medidos por cada proyecto ODS16';
ALTER TABLE mediciones_historicas COMMENT 'Historial de mediciones de indicadores ODS16';
ALTER TABLE auditoria_ods16         COMMENT 'Auditoría de cambios en el sistema ODS16';

SELECT 'Base de datos ODS16 creada exitosamente' AS mensaje, NOW() AS fecha_creacion;
