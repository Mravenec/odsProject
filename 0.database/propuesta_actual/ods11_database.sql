...
-- Esta base de datos se enfoca exclusivamente en la gestión de proyectos ODS11.
...
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_ods11 (tabla_afectada, registro_id, accion, usuario_id, valores_anteriores, valores_nuevos)
...
-- ────────────────────────────────────────────────────────────
-- VISTA: Estadísticas Generales ODS11 (metas específicas)
-- ────────────────────────────────────────────────────────────

CREATE VIEW vista_admin_estadisticas_generales AS
SELECT
    '11.1' AS meta_codigo,
    'Asegurar acceso vivienda adecuada servicios básicos' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.1.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '11.1.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_11_1,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.1.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_11_1_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 11

UNION ALL
SELECT
    '11.2' AS meta_codigo,
    'Proporcionar acceso transporte público sostenible' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.2.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '11.2.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_11_2,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.2.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_11_2_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 11
...
SELECT
    '11.3' AS meta_codigo,
    'Aumentar urbanización inclusiva sostenible' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.3.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '11.3.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_11_3,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.3.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_11_3_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 11
...
SELECT
    '11.4' AS meta_codigo,
    'Proteger patrimonio cultural natural' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.4.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '11.4.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_11_4,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.4.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_11_4_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 11
...
SELECT
    '11.5' AS meta_codigo,
    'Reducir desastres económicos humanos ambientales' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.5.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '11.5.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_11_5,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.5.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_11_5_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 11
...
SELECT
    '11.6' AS meta_codigo,
    'Reducir impacto ambiental ciudades per cápita' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.6.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '11.6.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_11_6,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.6.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_11_6_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 11
...
SELECT
    '11.7' AS meta_codigo,
    'Proporcionar acceso espacios públicos seguros inclusivos' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.7.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '11.7.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_11_7,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.7.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_11_7_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 11
...
SELECT
    '11.a' AS meta_codigo,
    'Fortalecer planeamiento urbano desarrollo sostenible' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.a.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '11.a.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_11_a,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.a.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_11_a_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 11
...
SELECT
    '11.b' AS meta_codigo,
    'Aplicar estrategias reducción riesgo desastres ciudades' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.b.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '11.b.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_11_b,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.b.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_11_b_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 11
...
SELECT
    '11.c' AS meta_codigo,
    'Apoyar países menos desarrollados construcción sostenible' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.c.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '11.c.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_11_c,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '11.c.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_11_c_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 11;
...
BEGIN
    SELECT 'TOTAL_PROYECTOS'     AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 11
    UNION ALL
    SELECT 'PROYECTOS_ACTIVOS'   AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 11 AND estado = 'activo'
    UNION ALL
    SELECT 'PROYECTOS_COMPLETADOS' AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 11 AND estado = 'completado'
    UNION ALL
    ...
    FROM indicadores i
    JOIN proyectos p ON i.proyecto_id = p.id
    WHERE p.objetivo_id = 11 AND (i.valor_actual / i.valor_meta) < 0.5
...
BEGIN
    ...
    FROM proyectos p
    JOIN ods_login.usuarios u ON p.usuario_id = u.id
    WHERE p.id = proyecto_id_param AND p.objetivo_id = 11;
...
CREATE INDEX idx_auditoria_fecha_tabla ON auditoria_ods11(fecha_cambio, tabla_afectada);

ALTER TABLE proyectos             COMMENT 'Proyectos ODS11 creados por usuarios';
ALTER TABLE metas_proyecto        COMMENT 'Metas específicas establecidas por cada proyecto ODS11';
ALTER TABLE indicadores           COMMENT 'Indicadores medidos por cada proyecto ODS11';
ALTER TABLE mediciones_historicas COMMENT 'Historial de mediciones de indicadores ODS11';
ALTER TABLE auditoria_ods11         COMMENT 'Auditoría de cambios en el sistema ODS11';

SELECT 'Base de datos ODS11 creada exitosamente' AS mensaje, NOW() AS fecha_creacion;
