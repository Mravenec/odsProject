...
-- Esta base de datos se enfoca exclusivamente en la gestión de proyectos ODS02.
...
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_ods02 (tabla_afectada, registro_id, accion, usuario_id, valores_anteriores, valores_nuevos)
...
-- ────────────────────────────────────────────────────────────
-- VISTA: Estadísticas Generales ODS02 (metas específicas)
-- ────────────────────────────────────────────────────────────

CREATE VIEW vista_admin_estadisticas_generales AS
SELECT
    '2.1' AS meta_codigo,
    'Poner fin hambre asegurar acceso alimentación sana nutritiva' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '2.1.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '2.1.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_2_1,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '2.1.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_2_1_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 2

UNION ALL
SELECT
    '2.2' AS meta_codigo,
    'Poner fin malnutrición retraso crecimiento emaciación niños' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '2.2.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '2.2.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_2_2,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '2.2.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_2_2_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 2
...
SELECT
    '2.3' AS meta_codigo,
    'Duplicar productividad agrícola ingresos productores alimentos' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '2.3.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '2.3.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_2_3,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '2.3.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_2_3_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 2
...
SELECT
    '2.4' AS meta_codigo,
    'Asegurar sostenibilidad sistemas producción alimentos' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '2.4.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '2.4.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_2_4,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '2.4.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_2_4_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 2
...
SELECT
    '2.5' AS meta_codigo,
    'Mantener diversidad genética semillas plantas animales' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '2.5.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '2.5.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_2_5,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '2.5.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_2_5_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 2
...
SELECT
    '2.a' AS meta_codigo,
    'Aumentar inversiones infraestructura rural investigación agrícola' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '2.a.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '2.a.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_2_a,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '2.a.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_2_a_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 2
...
SELECT
    '2.b' AS meta_codigo,
    'Corregir prevenir restricciones distorsiones comerciales mercados agropecuarios' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '2.b.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '2.b.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_2_b,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '2.b.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_2_b_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 2
...
SELECT
    '2.c' AS meta_codigo,
    'Adoptar medidas asegurar buen funcionamiento mercados productos básicos' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '2.c.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '2.c.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_2_c,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '2.c.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_2_c_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 2;
...
BEGIN
    SELECT 'TOTAL_PROYECTOS'     AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 2
    UNION ALL
    SELECT 'PROYECTOS_ACTIVOS'   AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 2 AND estado = 'activo'
    UNION ALL
    SELECT 'PROYECTOS_COMPLETADOS' AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 2 AND estado = 'completado'
    UNION ALL
    ...
    FROM indicadores i
    JOIN proyectos p ON i.proyecto_id = p.id
    WHERE p.objetivo_id = 2 AND (i.valor_actual / i.valor_meta) < 0.5
...
BEGIN
    ...
    FROM proyectos p
    JOIN ods_login.usuarios u ON p.usuario_id = u.id
    WHERE p.id = proyecto_id_param AND p.objetivo_id = 2;
...
ALTER TABLE proyectos             COMMENT 'Proyectos ODS02 creados por usuarios';
ALTER TABLE metas_proyecto        COMMENT 'Metas específicas establecidas por cada proyecto ODS02';
ALTER TABLE indicadores           COMMENT 'Indicadores medidos por cada proyecto ODS02';
ALTER TABLE mediciones_historicas COMMENT 'Historial de mediciones de indicadores ODS02';
ALTER TABLE auditoria_ods02         COMMENT 'Auditoría de cambios en el sistema ODS02';

SELECT 'Base de datos ODS02 creada exitosamente' AS mensaje, NOW() AS fecha_creacion;
