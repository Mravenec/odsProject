...
-- Esta base de datos se enfoca exclusivamente en la gestión de proyectos ODS04.
...
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_ods04 (tabla_afectada, registro_id, accion, usuario_id, valores_anteriores, valores_nuevos)
...
-- ────────────────────────────────────────────────────────────
-- VISTA: Estadísticas Generales ODS04 (metas específicas)
-- ────────────────────────────────────────────────────────────

CREATE VIEW vista_admin_estadisticas_generales AS
SELECT
    '4.1' AS meta_codigo,
    'Asegurar educación primaria secundaria gratuita equitativa calidad' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.1.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '4.1.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_4_1,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.1.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_4_1_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 4

UNION ALL
SELECT
    '4.2' AS meta_codigo,
    'Asegurar acceso atención desarrollo educación preescolar calidad' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.2.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '4.2.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_4_2,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.2.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_4_2_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 4
...
SELECT
    '4.3' AS meta_codigo,
    'Asegurar acceso igualitario formación técnica profesional superior' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.3.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '4.3.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_4_3,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.3.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_4_3_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 4
...
SELECT
    '4.4' AS meta_codigo,
    'Aumentar considerablemente número jóvenes adultos competencias necesarias' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.4.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '4.4.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_4_4,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.4.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_4_4_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 4
...
SELECT
    '4.5' AS meta_codigo,
    'Eliminar disparidades género educación acceso igualitario niveles enseñanza' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.5.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '4.5.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_4_5,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.5.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_4_5_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 4
...
SELECT
    '4.6' AS meta_codigo,
    'Asegurar alfabetización jóvenes adultos nociones aritmética' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.6.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '4.6.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_4_6,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.6.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_4_6_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 4
...
SELECT
    '4.7' AS meta_codigo,
    'Asegurar alumnos adquieran conocimientos promover desarrollo sostenible' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.7.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '4.7.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_4_7,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.7.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_4_7_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 4
...
SELECT
    '4.a' AS meta_codigo,
    'Construir adecuar instalaciones educativas necesidades niños discapacidad' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.a.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '4.a.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_4_a,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.a.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_4_a_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 4
...
SELECT
    '4.b' AS meta_codigo,
    'Aumentar considerablemente número becas países en desarrollo' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.b.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '4.b.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_4_b,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.b.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_4_b_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 4
...
SELECT
    '4.c' AS meta_codigo,
    'Aumentar considerablemente oferta docentes calificados países en desarrollo' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.c.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '4.c.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_4_c,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '4.c.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_4_c_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 4;
...
BEGIN
    SELECT 'TOTAL_PROYECTOS'     AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 4
    UNION ALL
    SELECT 'PROYECTOS_ACTIVOS'   AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 4 AND estado = 'activo'
    UNION ALL
    SELECT 'PROYECTOS_COMPLETADOS' AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 4 AND estado = 'completado'
    UNION ALL
    ...
    FROM indicadores i
    JOIN proyectos p ON i.proyecto_id = p.id
    WHERE p.objetivo_id = 4 AND (i.valor_actual / i.valor_meta) < 0.5
...
BEGIN
    ...
    FROM proyectos p
    JOIN ods_login.usuarios u ON p.usuario_id = u.id
    WHERE p.id = proyecto_id_param AND p.objetivo_id = 4;
...
CREATE INDEX idx_auditoria_fecha_tabla ON auditoria_ods04(fecha_cambio, tabla_afectada);

ALTER TABLE proyectos             COMMENT 'Proyectos ODS04 creados por usuarios';
ALTER TABLE metas_proyecto        COMMENT 'Metas específicas establecidas por cada proyecto ODS04';
ALTER TABLE indicadores           COMMENT 'Indicadores medidos por cada proyecto ODS04';
ALTER TABLE mediciones_historicas COMMENT 'Historial de mediciones de indicadores ODS04';
ALTER TABLE auditoria_ods04         COMMENT 'Auditoría de cambios en el sistema ODS04';

SELECT 'Base de datos ODS04 creada exitosamente' AS mensaje, NOW() AS fecha_creacion;
