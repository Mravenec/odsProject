...
-- Esta base de datos se enfoca exclusivamente en la gestión de proyectos ODS15.
...
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_ods15 (tabla_afectada, registro_id, accion, usuario_id, valores_anteriores, valores_nuevos)
...
-- ────────────────────────────────────────────────────────────
-- VISTA: Estadísticas Generales ODS15 (metas específicas)
-- ────────────────────────────────────────────────────────────

CREATE VIEW vista_admin_estadisticas_generales AS
SELECT
    '15.1' AS meta_codigo,
    'Conservar ecosistemas terrestres y agua dulce' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.1.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '15.1.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_15_1,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.1.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_15_1_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 15

UNION ALL
SELECT
    '15.2' AS meta_codigo,
    'Gestión sostenible de bosques' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.2.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '15.2.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_15_2,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.2.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_15_2_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 15
...
SELECT
    '15.3' AS meta_codigo,
    'Luchar contra desertificación' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.3.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '15.3.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_15_3,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.3.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_15_3_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 15
...
SELECT
    '15.4' AS meta_codigo,
    'Conservar ecosistemas montañosos' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.4.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '15.4.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_15_4,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.4.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_15_4_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 15
...
SELECT
    '15.5' AS meta_codigo,
    'Reducir degradación hábitats naturales' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.5.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '15.5.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_15_5,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.5.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_15_5_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 15
...
SELECT
    '15.6' AS meta_codigo,
    'Participación justa recursos genéticos' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.6.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '15.6.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_15_6,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.6.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_15_6_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 15
...
SELECT
    '15.7' AS meta_codigo,
    'Poner fin a caza furtiva y tráfico' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.7.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '15.7.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_15_7,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.7.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_15_7_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 15
...
SELECT
    '15.8' AS meta_codigo,
    'Prevenir especies exóticas invasoras' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.8.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '15.8.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_15_8,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.8.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_15_8_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 15
...
SELECT
    '15.9' AS meta_codigo,
    'Integrar valores ecosistemas en planificación' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.9.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '15.9.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_15_9,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.9.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_15_9_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 15
...
SELECT
    '15.a' AS meta_codigo,
    'Movilizar recursos para conservación biodiversidad' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.a.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '15.a.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_15_a,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.a.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_15_a_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 15
...
SELECT
    '15.b' AS meta_codigo,
    'Movilizar recursos para gestión forestal' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.b.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '15.b.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_15_b,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.b.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_15_b_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 15
...
SELECT
    '15.c' AS meta_codigo,
    'Aumentar apoyo contra caza furtiva' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.c.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '15.c.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_15_c,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '15.c.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_15_c_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 15;
...
BEGIN
    SELECT 'TOTAL_PROYECTOS'     AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 15
    UNION ALL
    SELECT 'PROYECTOS_ACTIVOS'   AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 15 AND estado = 'activo'
    UNION ALL
    SELECT 'PROYECTOS_COMPLETADOS' AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 15 AND estado = 'completado'
    UNION ALL
    ...
    FROM indicadores i
    JOIN proyectos p ON i.proyecto_id = p.id
    WHERE p.objetivo_id = 15 AND (i.valor_actual / i.valor_meta) < 0.5
...
BEGIN
    ...
    FROM proyectos p
    JOIN ods_login.usuarios u ON p.usuario_id = u.id
    WHERE p.id = proyecto_id_param AND p.objetivo_id = 15;
...
CREATE INDEX idx_auditoria_fecha_tabla ON auditoria_ods15(fecha_cambio, tabla_afectada);

ALTER TABLE proyectos             COMMENT 'Proyectos ODS15 creados por usuarios';
ALTER TABLE metas_proyecto        COMMENT 'Metas específicas establecidas por cada proyecto ODS15';
ALTER TABLE indicadores           COMMENT 'Indicadores medidos por cada proyecto ODS15';
ALTER TABLE mediciones_historicas COMMENT 'Historial de mediciones de indicadores ODS15';
ALTER TABLE auditoria_ods15         COMMENT 'Auditoría de cambios en el sistema ODS15';

SELECT 'Base de datos ODS15 creada exitosamente' AS mensaje, NOW() AS fecha_creacion;
