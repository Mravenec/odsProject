...
-- Esta base de datos se enfoca exclusivamente en la gestión de proyectos ODS12.
...
FOR EACH ROW
BEGIN
    INSERT INTO auditoria_ods12 (tabla_afectada, registro_id, accion, usuario_id, valores_anteriores, valores_nuevos)
...
-- ────────────────────────────────────────────────────────────
-- VISTA: Estadísticas Generales ODS12 (metas específicas)
-- ────────────────────────────────────────────────────────────

CREATE VIEW vista_admin_estadisticas_generales AS
SELECT
    '12.1' AS meta_codigo,
    'Aplicar marco consumo producción sostenibles' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.1.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '12.1.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_12_1,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.1.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_12_1_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 12

UNION ALL
SELECT
    '12.2' AS meta_codigo,
    'Lograr gestión sostenible recursos naturales' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.2.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '12.2.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_12_2,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.2.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_12_2_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 12
...
SELECT
    '12.3' AS meta_codigo,
    'Reducir desperdicio alimentos per cápita' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.3.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '12.3.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_12_3,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.3.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_12_3_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 12
...
SELECT
    '12.4' AS meta_codigo,
    'Gestión ecológica productos químicos desechos' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.4.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '12.4.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_12_4,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.4.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_12_4_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 12
...
SELECT
    '12.5' AS meta_codigo,
    'Reducir generación desechos reciclado reutilización' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.5.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '12.5.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_12_5,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.5.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_12_5_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 12
...
SELECT
    '12.6' AS meta_codigo,
    'Promover prácticas sostenibles empresas' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.6.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '12.6.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_12_6,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.6.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_12_6_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 12
...
SELECT
    '12.7' AS meta_codigo,
    'Promover adquisiciones públicas sostenibles' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.7.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '12.7.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_12_7,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.7.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_12_7_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 12
...
SELECT
    '12.8' AS meta_codigo,
    'Asegurar información conocimientos desarrollo sostenible' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.8.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '12.8.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_12_8,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.8.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_12_8_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 12
...
SELECT
    '12.a' AS meta_codigo,
    'Fortalecer capacidad científica tecnológica países en desarrollo' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.a.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '12.a.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_12_a,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.a.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_12_a_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 12
...
SELECT
    '12.b' AS meta_codigo,
    'Aplicar instrumentos contabilidad turismo sostenible' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.b.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '12.b.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_12_b,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.b.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_12_b_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 12
...
SELECT
    '12.c' AS meta_codigo,
    'Racionalizar subsidios ineficientes combustibles fósiles' AS meta_descripcion,
    COUNT(DISTINCT p.id) AS total_proyectos,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.c.%' THEN p.id END) AS proyectos_con_indicadores,
    AVG(CASE WHEN i.indicador_codigo LIKE '12.c.%' THEN (i.valor_actual / i.valor_meta) * 100 END) AS promedio_logro_meta_12_c,
    COUNT(DISTINCT CASE WHEN i.indicador_codigo LIKE '12.c.%' AND i.valor_actual >= i.valor_meta THEN i.id END) AS indicadores_meta_12_c_logrados
FROM proyectos p
LEFT JOIN indicadores i ON p.id = i.proyecto_id
WHERE p.objetivo_id = 12;
...
BEGIN
    SELECT 'TOTAL_PROYECTOS'     AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 12
    UNION ALL
    SELECT 'PROYECTOS_ACTIVOS'   AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 12 AND estado = 'activo'
    UNION ALL
    SELECT 'PROYECTOS_COMPLETADOS' AS metrica, COUNT(*) AS valor FROM proyectos WHERE objetivo_id = 12 AND estado = 'completado'
    UNION ALL
    ...
    FROM indicadores i
    JOIN proyectos p ON i.proyecto_id = p.id
    WHERE p.objetivo_id = 12 AND (i.valor_actual / i.valor_meta) < 0.5
...
BEGIN
    ...
    FROM proyectos p
    JOIN ods_login.usuarios u ON p.usuario_id = u.id
    WHERE p.id = proyecto_id_param AND p.objetivo_id = 12;
...
CREATE INDEX idx_auditoria_fecha_tabla ON auditoria_ods12(fecha_cambio, tabla_afectada);

ALTER TABLE proyectos             COMMENT 'Proyectos ODS12 creados por usuarios';
ALTER TABLE metas_proyecto        COMMENT 'Metas específicas establecidas por cada proyecto ODS12';
ALTER TABLE indicadores           COMMENT 'Indicadores medidos por cada proyecto ODS12';
ALTER TABLE mediciones_historicas COMMENT 'Historial de mediciones de indicadores ODS12';
ALTER TABLE auditoria_ods12         COMMENT 'Auditoría de cambios en el sistema ODS12';

SELECT 'Base de datos ODS12 creada exitosamente' AS mensaje, NOW() AS fecha_creacion;
