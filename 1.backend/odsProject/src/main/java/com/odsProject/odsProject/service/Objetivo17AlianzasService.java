...
public class Objetivo17AlianzasService implements IObjetivo17AlianzasService {
    ...
    private static final Logger log = LoggerFactory.getLogger(Objetivo17AlianzasService.class);
    ...
    @Override public ProyectoIndicadores saveIndicador(ProyectoIndicadores indicador) {
        ProyectoIndicadores saved = objetivo17AlianzasRepository.saveIndicador(indicador);
    ...
    @Override public ProyectoIndicadorParametros saveMetaProyecto(ProyectoIndicadorParametros meta) { 
        ProyectoIndicadorParametros saved = objetivo17AlianzasRepository.saveMetaProyecto(meta);
    ...
    @Override public ProyectoIndicadorParametros updateMetaProyecto(ProyectoIndicadorParametros meta) { 
        ProyectoIndicadorParametros updated = objetivo17AlianzasRepository.updateMetaProyecto(meta);
    ...
    private void recalculateIndicator(Integer proyectoIndicadorId) {
        ...
        Optional<ProyectoIndicadores> optIndicador = objetivo17AlianzasRepository.findIndicadorByIdEntity(proyectoIndicadorId);
        ...
        List<ProyectoIndicadorParametros> parametros = objetivo17AlianzasRepository.findMetasByProyectoIndicador(proyectoIndicadorId);
        ...
        try {
            ...
            objetivo17AlianzasRepository.updateIndicador(indicador);
        } catch (Exception e) {
            log.warn("Error recalculando indicador ODS17 " + proyectoIndicadorId + ": " + e.getMessage());
        }
    }
    ...
    @Override
    @org.springframework.transaction.annotation.Transactional("txManagerOds17")
    public java.util.Map<String, Object> saveMedicionAuditada(java.util.Map<String, Object> payload) {
        ...
        if (__pidCheck != null) {
            objetivo17AlianzasRepository.findIndicadorByIdEntity(__pidCheck).ifPresent(__ind -> {
        ...
        Optional<ProyectoIndicadores> optInd = objetivo17AlianzasRepository.findIndicadorByIdEntity(proyectoIndicadorId);
        if (optInd.isEmpty())
        ...
        java.util.List<ProyectoIndicadorParametros> parametros =
            objetivo17AlianzasRepository.findMetasByProyectoIndicador(proyectoIndicadorId);
        ...
        MedicionesHistoricas saved = objetivo17AlianzasRepository.saveMedicion(medicion);
    ...
    public java.util.Map<String, Object> getMedicionAuditoria(Integer medicionId) {
        ...
        MedicionesHistoricas medicion = objetivo17AlianzasRepository.findMedicionByIdEntity(medicionId);
        if (medicion == null) throw new IllegalArgumentException("Medición no encontrada: " + medicionId);

        Optional<ProyectoIndicadores> optInd =
            objetivo17AlianzasRepository.findIndicadorByIdEntity(medicion.getProyectoIndicadorId());
        ProyectoIndicadores indicador = optInd.orElse(null);

        java.util.List<java.util.Map<String, Object>> valores =
            objetivo17AlianzasRepository.findMedicionParametroValoresByMedicion(medicionId);
    ...
    private void seedParametrosFromFormula(Integer proyectoIndicadorId, String formula) {
        ...
        for (String v : variables) {
            ...
            try {
                objetivo17AlianzasRepository.saveMetaProyecto(nuevo);
            } catch (Exception ex) {
