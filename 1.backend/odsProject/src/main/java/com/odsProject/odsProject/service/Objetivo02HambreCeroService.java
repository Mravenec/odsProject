...
public class Objetivo02HambreCeroService implements IObjetivo02HambreCeroService {
    ...
    @Override public ProyectoIndicadores saveIndicador(ProyectoIndicadores indicador) {
        ProyectoIndicadores saved = objetivo02HambreCeroRepository.saveIndicador(indicador);
    ...
    public java.util.Map<String, Object> saveMedicionAuditada(java.util.Map<String, Object> payload) {
        ...
        Optional<ProyectoIndicadores> optInd = objetivo02HambreCeroRepository.findIndicadorByIdEntity(proyectoIndicadorId);
        if (optInd.isEmpty())
        ...
        java.util.List<ProyectoIndicadorParametros> parametros =
            objetivo02HambreCeroRepository.findMetasByProyectoIndicador(proyectoIndicadorId);
        ...
        MedicionesHistoricas saved = objetivo02HambreCeroRepository.saveMedicion(medicion);

        for (java.util.Map.Entry<Integer, java.math.BigDecimal> e : valoresPorParametroId.entrySet()) {
            objetivo02HambreCeroRepository.insertMedicionParametroValor(saved.getId(), e.getKey(), e.getValue());
            ProyectoIndicadorParametros p = paramsById.get(e.getKey());
            if (p != null) {
                p.setValorActual(e.getValue());
                objetivo02HambreCeroRepository.updateMetaProyecto(p);
            }
        }
    ...
    public java.util.Map<String, Object> getMedicionAuditoria(Integer medicionId) {
        ...
        MedicionesHistoricas medicion = objetivo02HambreCeroRepository.findMedicionByIdEntity(medicionId);
        if (medicion == null) throw new IllegalArgumentException("Medición no encontrada: " + medicionId);

        Optional<ProyectoIndicadores> optInd =
            objetivo02HambreCeroRepository.findIndicadorByIdEntity(medicion.getProyectoIndicadorId());
        ProyectoIndicadores indicador = optInd.orElse(null);

        java.util.List<java.util.Map<String, Object>> valores =
            objetivo02HambreCeroRepository.findMedicionParametroValoresByMedicion(medicionId);
