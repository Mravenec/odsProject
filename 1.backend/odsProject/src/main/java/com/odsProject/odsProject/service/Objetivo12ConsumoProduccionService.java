...
public class Objetivo12ConsumoProduccionService implements IObjetivo12ConsumoProduccionService {
    ...
    @Override public ProyectoIndicadores saveIndicador(ProyectoIndicadores indicador) {
        ProyectoIndicadores saved = objetivo12ConsumoProduccionRepository.saveIndicador(indicador);
    ...
    public java.util.Map<String, Object> saveMedicionAuditada(java.util.Map<String, Object> payload) {
        ...
        if (__pidCheck != null) {
            objetivo12ConsumoProduccionRepository.findIndicadorByIdEntity(__pidCheck).ifPresent(__ind -> {
        ...
        Optional<ProyectoIndicadores> optInd = objetivo12ConsumoProduccionRepository.findIndicadorByIdEntity(proyectoIndicadorId);
        if (optInd.isEmpty())
        ...
        java.util.List<ProyectoIndicadorParametros> parametros =
            objetivo12ConsumoProduccionRepository.findMetasByProyectoIndicador(proyectoIndicadorId);
        ...
        MedicionesHistoricas saved = objetivo12ConsumoProduccionRepository.saveMedicion(medicion);
    ...
    public java.util.Map<String, Object> getMedicionAuditoria(Integer medicionId) {
        ...
        MedicionesHistoricas medicion = objetivo12ConsumoProduccionRepository.findMedicionByIdEntity(medicionId);
        if (medicion == null) throw new IllegalArgumentException("Medición no encontrada: " + medicionId);

        Optional<ProyectoIndicadores> optInd =
            objetivo12ConsumoProduccionRepository.findIndicadorByIdEntity(medicion.getProyectoIndicadorId());
        ProyectoIndicadores indicador = optInd.orElse(null);

        java.util.List<java.util.Map<String, Object>> valores =
            objetivo12ConsumoProduccionRepository.findMedicionParametroValoresByMedicion(medicionId);
