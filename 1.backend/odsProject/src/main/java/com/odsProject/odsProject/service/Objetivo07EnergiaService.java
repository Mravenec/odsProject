...
public class Objetivo07EnergiaService implements IObjetivo07EnergiaService {
    ...
    @Override public ProyectoIndicadores saveIndicador(ProyectoIndicadores indicador) {
        ProyectoIndicadores saved = objetivo07EnergiaRepository.saveIndicador(indicador);
    ...
    public java.util.Map<String, Object> saveMedicionAuditada(java.util.Map<String, Object> payload) {
        ...
        if (__pidCheck != null) {
            objetivo07EnergiaRepository.findIndicadorByIdEntity(__pidCheck).ifPresent(__ind -> {
        ...
        Optional<ProyectoIndicadores> optInd = objetivo07EnergiaRepository.findIndicadorByIdEntity(proyectoIndicadorId);
        if (optInd.isEmpty())
        ...
        java.util.List<ProyectoIndicadorParametros> parametros =
            objetivo07EnergiaRepository.findMetasByProyectoIndicador(proyectoIndicadorId);
        ...
        MedicionesHistoricas saved = objetivo07EnergiaRepository.saveMedicion(medicion);
    ...
    public java.util.Map<String, Object> getMedicionAuditoria(Integer medicionId) {
        ...
        MedicionesHistoricas medicion = objetivo07EnergiaRepository.findMedicionByIdEntity(medicionId);
        if (medicion == null) throw new IllegalArgumentException("Medición no encontrada: " + medicionId);

        Optional<ProyectoIndicadores> optInd =
            objetivo07EnergiaRepository.findIndicadorByIdEntity(medicion.getProyectoIndicadorId());
        ProyectoIndicadores indicador = optInd.orElse(null);

        java.util.List<java.util.Map<String, Object>> valores =
            objetivo07EnergiaRepository.findMedicionParametroValoresByMedicion(medicionId);
