...
public class Objetivo10ReduccionDesigualdadService implements IObjetivo10ReduccionDesigualdadService {
    ...
    private static final Logger log = LoggerFactory.getLogger(Objetivo10ReduccionDesigualdadService.class);
    ...
    @Override public ProyectoIndicadores saveIndicador(ProyectoIndicadores indicador) {
        ProyectoIndicadores saved = objetivo10ReduccionDesigualdadRepository.saveIndicador(indicador);
    ...
    private void recalculateIndicator(Integer proyectoIndicadorId) {
        ...
        try {
            ...
            log.warn("Error recalculando indicador ODS10 " + proyectoIndicadorId + ": " + e.getMessage());
        }
    }
    ...
    @Override
    @org.springframework.transaction.annotation.Transactional("txManagerOds10")
    public java.util.Map<String, Object> saveMedicionAuditada(java.util.Map<String, Object> payload) {
        ...
        if (__pidCheck != null) {
            objetivo10ReduccionDesigualdadRepository.findIndicadorByIdEntity(__pidCheck).ifPresent(__ind -> {
        ...
        Optional<ProyectoIndicadores> optInd = objetivo10ReduccionDesigualdadRepository.findIndicadorByIdEntity(proyectoIndicadorId);
        if (optInd.isEmpty())
        ...
        java.util.List<ProyectoIndicadorParametros> parametros =
            objetivo10ReduccionDesigualdadRepository.findMetasByProyectoIndicador(proyectoIndicadorId);
        ...
        MedicionesHistoricas saved = objetivo10ReduccionDesigualdadRepository.saveMedicion(medicion);

        for (java.util.Map.Entry<Integer, java.math.BigDecimal> e : valoresPorParametroId.entrySet()) {
            objetivo10ReduccionDesigualdadRepository.insertMedicionParametroValor(saved.getId(), e.getKey(), e.getValue());
            ProyectoIndicadorParametros p = paramsById.get(e.getKey());
            if (p != null) {
                p.setValorActual(e.getValue());
                objetivo10ReduccionDesigualdadRepository.updateMetaProyecto(p);
            }
        }
    ...
    public java.util.Map<String, Object> getMedicionAuditoria(Integer medicionId) {
        ...
        MedicionesHistoricas medicion = objetivo10ReduccionDesigualdadRepository.findMedicionByIdEntity(medicionId);
        if (medicion == null) throw new IllegalArgumentException("Medición no encontrada: " + medicionId);

        Optional<ProyectoIndicadores> optInd =
            objetivo10ReduccionDesigualdadRepository.findIndicadorByIdEntity(medicion.getProyectoIndicadorId());
        ProyectoIndicadores indicador = optInd.orElse(null);

        java.util.List<java.util.Map<String, Object>> valores =
            objetivo10ReduccionDesigualdadRepository.findMedicionParametroValoresByMedicion(medicionId);
    ...
    private void seedParametrosFromFormula(Integer proyectoIndicadorId, String formula) {
        ...
        for (String v : variables) {
            ...
            try {
                objetivo10ReduccionDesigualdadRepository.saveMetaProyecto(nuevo);
            } catch (Exception ex) {
